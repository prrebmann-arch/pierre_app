...
/**
 * Unified Stripe CRON handler
 * - /api/stripe-cron?action=invoice  → Monthly invoice (10th)
 * - /api/stripe-cron?action=retry    → Daily retry
 * - /api/stripe-cron (no action)     → Runs both (for daily cron)
 */

const { verifyCronSecret, handleAuthError } = require('./_auth');
...
async function runInvoice(supabase, now) {
  ...
  const results = { invoiced: 0, skipped: 0, errors: [] };
  ...
  for (const coach of coaches) {
    try {
      ...
      const uniqueAthletes = [...new Set((allActivities || []).map(a => a.athlete_id))];
      const detail = [];

      for (const athleteId of uniqueAthletes) {
        const acts = (allActivities || []).filter(a => a.athlete_id === athleteId);
        let activeStart = null, totalDays = 0;

        for (const act of acts) {
          const actDate = new Date(act.event_date);
          if (act.event === 'added') { activeStart = actDate; }
          else if (act.event === 'removed' && activeStart) {
            const s = new Date(Math.max(activeStart, new Date(monthStart)));
            const e = new Date(Math.min(actDate, new Date(monthEnd)));
            if (e > s) totalDays += Math.ceil((e - s) / (86400000));
            activeStart = null;
          }
        }
        if (activeStart) {
          ...
          if (e >= s) totalDays += Math.ceil((e - s) / 86400000) + 1;
        }
      ...
      let stripeInvoiceId = null, status = 'pending';
      ...
      if (coach.stripe_customer_id && coach.has_payment_method) {
        ...
        } catch (e) { status = 'failed'; results.errors.push({ coach: coach.user_id, error: e.message }); }
      }
      ...
      results.invoiced++;
    } catch (e) { results.errors.push({ coach: coach.user_id, error: e.message }); }
  }
  return results;
}

// ── RETRY (runs daily) ──
async function runRetry(supabase, now) {
  const results = { retried: 0, blocked: 0, errors: [] };
  ...
  for (const inv of invoices) {
    try {
      ...
      if (retryCount >= 3) {
        const daysSince = Math.floor((now - new Date(inv.created_at)) / 86400000);
        if (daysSince >= 30) {
      ...
      if (inv.stripe_invoice_id) {
        try { const p = await stripe.invoices.pay(inv.stripe_invoice_id); paid = p.status === 'paid'; } catch {}
      }
      ...
      if (paid) {
        ...
        await supabase.from('notifications').insert({ user_id: inv.coach_id, type: 'billing', title: 'Paiement réussi', body: `${(inv.total_amount / 100).toFixed(2)}€ prélevé.` }).catch(() => {});
      } else {
        const statusMap = { 0: 'retry_1', 1: 'retry_2', 2: 'retry_3' };
        ...
        await supabase.from('notifications').insert({ user_id: inv.coach_id, type: 'billing', title: 'Échec prélèvement', body: `Nouvelle tentative dans ${nextDays}j.` }).catch(() => {});
      }

      results.retried++;
    } catch (e) { results.errors.push({ invoice: inv.id, error: e.message }); }
  }
  return results;
}
